...
describe('Municipality homepage', () => {
  ...
  const IvisitHomepage = (municipality, language = false, userType = false) => {
    ...
    const url = '/municipality-'+ municipality + '/node/' + municipality + ((userType || language) ? '/?' : '') + ((userType) ? ('user_type=' + userType) : '') + ((userType && language) ? '&' : '') + (((language) ? ('language=' + languageText[language]) : ''));
    browser.url(url);
  };
