...
/**
 * @file
 * Contains \MuniMigrateBase.
 */

/**
 * Class MuniMigrateBase.
 */
abstract class MuniMigrateBase extends Migration {
  ...
  protected function getMigrateDirectory() {
    return variable_get('muni_migrate_directory', FALSE) ? variable_get('muni_migrate_directory') : drupal_get_path('module', 'muni_migrate');
  }

  /**
   * MuniMigrateBase constructor.
   */
  public function __construct($arguments) {
    ...
    $destination_classes = [
      ...
      'multifield' => 'MuniMigrateDestinationMultifield',
    ];
    ...
    if ($this->entityType == 'node') {
      ...
      $this->dependencies = ['MuniMigrateUsers'];

      $this
        ->addFieldMapping('uid', 'author')
        ->sourceMigration('MuniMigrateUsers')
        ->defaultValue(1);
