 ...
 */
function muni_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function muni_install_tasks() {
  $tasks = array();

  $tasks['muni_setup_variables'] = array(
    'display_name' => st('Set Variables'),
    'display' => FALSE,
  );

  // Run this as the last task!
  $tasks['muni_setup_rebuild_permissions'] = array(
 ...
 */
function muni_setup_variables() {
  $variables = array(
    // Features default export path.
    'features_default_export_path' => 'profiles/muni/modules/custom',
    // Mime-mail.
    'mimemail_format' => 'full_html',
    'mimemail_sitestyle' => FALSE,
    'mimemail_name' => 'Muni',
    'mimemail_mail' => 'info@muni.com',
    // jQuery versions.
 ...
 * installation.
 */
function muni_setup_rebuild_permissions() {
  node_access_rebuild();
}
