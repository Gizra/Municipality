...
cd "$ROOT_DIR"/server
php ./www/scripts/run-tests.sh --cache --cache-modules --php "$(which php)" --concurrency 2 --verbose --color --url http://server.local Hedley 2>&1 | tee /tmp/simpletest-result.txt
